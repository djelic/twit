...
Twitter.prototype.getRequestToken = function (params) {
  var self = this
  return new Promise(function (resolve, reject) {
    request.post({
      url: endpoints.OA_REQ,
      oauth: {
        consumer_key: self.config.consumer_key,
        consumer_secret: self.config.consumer_secret,
      ...
      if (response.statusCode !== 200) {
        return reject(wrapTokenError(response))
      }
...
Twitter.prototype.generateAuthUrl = function (params) {
  return this.getRequestToken(params).then(function (token) {
    return 'https://twitter.com/oauth/authenticate?oauth_token=' + token.oauth_token
  })
}

Twitter.prototype.getAccessToken = function (params) {
  var self = this
  return new Promise(function (resolve, reject) {
    request.post({
      url: endpoints.OA_ACCESS,
      oauth: {
        consumer_key: self.config.consumer_key,
        consumer_secret: self.config.consumer_secret,
        token: params.token,
...
function wrapTokenError (response) {
